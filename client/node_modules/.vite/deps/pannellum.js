...
window.libpannellum = function(E, g, p) {
  function Ba(K) {
    function ja(a2, d) {
      return 1 == a2.level && 1 != d.level ? -1 : 1 == d.level && 1 != a2.level ? 1 : d.timestamp - a2.timestamp;
    }
    function Q(a2, d) {
      return a2.level != d.level ? a2.level - d.level : a2.diff - d.diff;
    }
    function ka(a2, d, c2, g2, k, h) {
      this.vertices = a2;
      this.side = d;
      this.level = c2;
      this.x = g2;
      this.y = k;
      this.path = h.replace("%s", d).replace("%l", c2).replace("%x", g2).replace("%y", k);
    }
    function Ja(a2, d, g2, p2, k) {
      var h;
      var e = d.vertices;
      h = la(a2, e.slice(0, 3));
      var u = la(a2, e.slice(3, 6)), x = la(a2, e.slice(6, 9)), e = la(a2, e.slice(
        9,
        12
      )), t = h[0] + u[0] + x[0] + e[0];
      -4 == t || 4 == t ? h = false : (t = h[1] + u[1] + x[1] + e[1], h = -4 == t || 4 == t ? false : 4 != h[2] + u[2] + x[2] + e[2]);
      if (h) {
        h = d.vertices;
        u = h[0] + h[3] + h[6] + h[9];
        x = h[1] + h[4] + h[7] + h[10];
        ...
        u = Math.abs(u);
        d.diff = Math.acos(Math.sin(g2) * Math.sin(e) + Math.cos(g2) * Math.cos(e) * Math.cos(u));
        u = false;
        for (x = 0; x < c.nodeCache.length; x++) if (c.nodeCache[x].path == d.path) {
          u = true;
        ...
        u || (d.timestamp = c.nodeCacheTimestamp++, c.currentNodes.push(d), c.nodeCache.push(d));
        if (d.level < c.level) {
          var e = m.cubeResolution * Math.pow(2, d.level - m.maxLevel), u = Math.ceil(e * m.invTileResolution) - 1, x = e % m.tileResolution * 2, l2 = 2 * e % m.tileResolution;
          0 === l2 && (l2 = m.tileResolution);
          0 === x && (x = 2 * m.tileResolution);
          t = 0.5;
          if (d.x == u || d.y == u) t = 1 - m.tileResolution / (m.tileResolution + l2);
          var y = 1 - t, e = [], s2 = t, z2 = t, D = t, F = y, A2 = y, B = y;
          if (l2 < m.tileResolution) if (d.x == u && d.y != u) {
            if (A2 = z2 = 0.5, "d" == d.side || "u" == d.side) B = D = 0.5;
          } else d.x != u && d.y == u && (F = s2 = 0.5, "l" == d.side || "r" == d.side) && (B = D = 0.5);
          x <= m.tileResolution && (d.x == u && (s2 = 0, F = 1, "l" == d.side || "r" == d.side) && (D = 0, B = 1), d.y == u && (z2 = 0, A2 = 1, "d" == d.side || "u" == d.side) && (D = 0, B = 1));
          l2 = [h[0], h[1], h[2], h[0] * s2 + h[3] * F, h[1] * t + h[4] * y, h[2] * D + h[5] * B, h[0] * s2 + h[6] * F, h[1] * z2 + h[7] * A2, h[2] * D + h[8] * B, h[0] * t + h[9] * y, h[1] * z2 + h[10] * A2, h[2] * D + h[11] * B];
          l2 = new ka(l2, d.side, d.level + 1, 2 * d.x, 2 * d.y, m.fullpath);
          e.push(l2);
          d.x == u && x <= m.tileResolution || (l2 = [h[0] * s2 + h[3] * F, h[1] * t + h[4] * y, h[2] * D + h[5] * B, h[3], h[4], h[5], h[3] * t + h[6] * y, h[4] * z2 + h[7] * A2, h[5] * D + h[8] * B, h[0] * s2 + h[6] * F, h[1] * z2 + h[7] * A2, h[2] * D + h[8] * B], l2 = new ka(l2, d.side, d.level + 1, 2 * d.x + 1, 2 * d.y, m.fullpath), e.push(l2));
          d.x == u && x <= m.tileResolution || d.y == u && x <= m.tileResolution || (l2 = [h[0] * s2 + h[6] * F, h[1] * z2 + h[7] * A2, h[2] * D + h[8] * B, h[3] * t + h[6] * y, h[4] * z2 + h[7] * A2, h[5] * D + h[8] * B, h[6], h[7], h[8], h[9] * s2 + h[6] * F, h[10] * t + h[7] * y, h[11] * D + h[8] * B], l2 = new ka(l2, d.side, d.level + 1, 2 * d.x + 1, 2 * d.y + 1, m.fullpath), e.push(l2));
          d.y == u && x <= m.tileResolution || (l2 = [h[0] * t + h[9] * y, h[1] * z2 + h[10] * A2, h[2] * D + h[11] * B, h[0] * s2 + h[6] * F, h[1] * z2 + h[7] * A2, h[2] * D + h[8] * B, h[9] * s2 + h[6] * F, h[10] * t + h[7] * y, h[11] * D + h[8] * B, h[9], h[10], h[11]], l2 = new ka(l2, d.side, d.level + 1, 2 * d.x, 2 * d.y + 1, m.fullpath), e.push(l2));
          for (d = 0; d < e.length; d++) Ja(a2, e[d], g2, p2, k);
        }
      }
    }
    function ta() {
    ...
    function ua(a2, d, c2) {
      var g2 = Math.sin(d);
      d = Math.cos(d);
      if ("x" == c2) return [a2[0], d * a2[1] + g2 * a2[2], d * a2[2] - g2 * a2[1], a2[3], d * a2[4] + g2 * a2[5], d * a2[5] - g2 * a2[4], a2[6], d * a2[7] + g2 * a2[8], d * a2[8] - g2 * a2[7]];
      if ("y" == c2) return [d * a2[0] - g2 * a2[2], a2[1], d * a2[2] + g2 * a2[0], d * a2[3] - g2 * a2[5], a2[4], d * a2[5] + g2 * a2[3], d * a2[6] - g2 * a2[8], a2[7], d * a2[8] + g2 * a2[6]];
      if ("z" == c2) return [d * a2[0] + g2 * a2[1], d * a2[1] - g2 * a2[0], a2[2], d * a2[3] + g2 * a2[4], d * a2[4] - g2 * a2[3], a2[5], d * a2[6] + g2 * a2[7], d * a2[7] - g2 * a2[6], a2[8]];
    }
    function ma(a2) {
    ...
    function la(a2, d) {
      var c2 = [a2[0] * d[0] + a2[1] * d[1] + a2[2] * d[2], a2[4] * d[0] + a2[5] * d[1] + a2[6] * d[2], a2[11] + a2[8] * d[0] + a2[9] * d[1] + a2[10] * d[2], 1 / (a2[12] * d[0] + a2[13] * d[1] + a2[14] * d[2])], g2 = c2[0] * c2[3], k = c2[1] * c2[3], c2 = c2[2] * c2[3], h = [0, 0, 0];
      -1 > g2 && (h[0] = -1);
      1 < g2 && (h[0] = 1);
      -1 > k && (h[1] = -1);
      1 < k && (h[1] = 1);
    ...
    this.init = function(L, d, Ca, I, k, h, e, u) {
      function x(a2) {
        if (E2) {
          var d2 = a2 * a2 * 4, h2 = new Uint8ClampedArray(d2), c2 = u.backgroundColor ? u.backgroundColor : [0, 0, 0];
          c2[0] *= 255;
          c2[1] *= 255;
          c2[2] *= 255;
          for (var b = 0; b < d2; b++) h2[b++] = c2[0], h2[b++] = c2[1], h2[b++] = c2[2];
      ...
      if (!a && ("multires" == z && m.hasOwnProperty("fallbackPath") || "cubemap" == z) && ("WebkitAppearance" in g.documentElement.style || navigator.userAgent.match(/Trident.*rv[ :]*11\./) || -1 !== navigator.appVersion.indexOf("MSIE 10"))) {
        ...
        k = function() {
          ...
          R.appendChild(a2);
          var d2 = a2.getContext("2d");
          a2.style.width = this.width + 4 + "px";
          a2.style.height = this.height + 4 + "px";
          a2.width = this.width + 4;
          a2.height = this.height + 4;
          d2.drawImage(this, 2, 2);
          var h2 = d2.getImageData(0, 0, a2.width, a2.height), c2 = h2.data, b, e2;
          for (b = 2; b < a2.width - 2; b++) for (e2 = 0; 4 > e2; e2++) c2[4 * (b + a2.width) + e2] = c2[4 * (b + 2 * a2.width) + e2], c2[4 * (b + a2.width * (a2.height - 2)) + e2] = c2[4 * (b + a2.width * (a2.height - 3)) + e2];
          for (b = 2; b < a2.height - 2; b++) for (e2 = 0; 4 > e2; e2++) c2[4 * (b * a2.width + 1) + e2] = c2[4 * (b * a2.width + 2) + e2], c2[4 * ((b + 1) * a2.width - 2) + e2] = c2[4 * ((b + 1) * a2.width - 3) + e2];
          for (e2 = 0; 4 > e2; e2++) c2[4 * (a2.width + 1) + e2] = c2[4 * (2 * a2.width + 2) + e2], c2[4 * (2 * a2.width - 2) + e2] = c2[4 * (3 * a2.width - 3) + e2], c2[4 * (a2.width * (a2.height - 2) + 1) + e2] = c2[4 * (a2.width * (a2.height - 3) + 2) + e2], c2[4 * (a2.width * (a2.height - 1) - 2) + e2] = c2[4 * (a2.width * (a2.height - 2) - 3) + e2];
          for (b = 1; b < a2.width - 1; b++) for (e2 = 0; 4 > e2; e2++) c2[4 * b + e2] = c2[4 * (b + a2.width) + e2], c2[4 * (b + a2.width * (a2.height - 1)) + e2] = c2[4 * (b + a2.width * (a2.height - 2)) + e2];
          for (b = 1; b < a2.height - 1; b++) for (e2 = 0; 4 > e2; e2++) c2[b * a2.width * 4 + e2] = c2[4 * (b * a2.width + 1) + e2], c2[4 * ((b + 1) * a2.width - 1) + e2] = c2[4 * ((b + 1) * a2.width - 2) + e2];
          for (e2 = 0; 4 > e2; e2++) c2[e2] = c2[4 * (a2.width + 1) + e2], c2[4 * (a2.width - 1) + e2] = c2[4 * (2 * a2.width - 2) + e2], c2[a2.width * (a2.height - 1) * 4 + e2] = c2[4 * (a2.width * (a2.height - 2) + 1) + e2], c2[4 * (a2.width * a2.height - 1) + e2] = c2[4 * (a2.width * (a2.height - 1) - 2) + e2];
          d2.putImageData(h2, 0, 0);
          D.call(this);
        };
        var D = function() {
          0 < this.width ? ($ === p && ($ = this.width), $ != this.width && console.log("Fallback faces have inconsistent widths: " + $ + " vs. " + this.width)) : E2 = true;
          S2++;
          6 == S2 && ($ = this.width, K.appendChild(R), e());
        }, E2 = false;
        for (t = 0; 6 > t; t++) h = new Image(), h.crossOrigin = va.crossOrigin ? va.crossOrigin : "anonymous", h.side = t, h.onload = k, h.onerror = D, h.src = "multires" == z ? I.replace("%s", Q2[t]) + "." + m.extension : m[t].src;
        x($);
      } else {
        ...
        a.getShaderPrecisionFormat && (d = a.getShaderPrecisionFormat(a.FRAGMENT_SHADER, a.HIGH_FLOAT)) && 1 > d.precision && (oa = oa.replace("highp", "mediump"));
        ...
        d = pa;
        "cubemap" == z ? (y = a.TEXTURE_CUBE_MAP, d = qa) : "multires" == z && (d = bb);
        a.shaderSource(V, d);
        a.compileShader(V);
        c = a.createProgram();
        a.attachShader(c, U);
        ...
        I = a.getError();
        if (0 !== I) throw console.log("Error: Something went wrong with WebGL!", I), { type: "webgl error" };
        e();
      }
    };
    ...
    this.render = function(g2, d, l2, s2) {
      var k, h = 0;
      s2 === p && (s2 = {});
      s2.roll && (h = s2.roll);
      if (ga !== p) {
        k = ga[0];
        var e = ga[1], u = g2, x = d, t = Math.cos(e) * Math.sin(g2) * Math.sin(k) + Math.cos(g2) * (Math.cos(k) * Math.cos(d) + Math.sin(e) * Math.sin(k) * Math.sin(d)), E2 = -Math.sin(g2) * Math.sin(e) + Math.cos(g2) * Math.cos(e) * Math.sin(d);
        g2 = Math.cos(e) * Math.cos(k) * Math.sin(g2) + Math.cos(g2) * (-Math.cos(d) * Math.sin(k) + Math.cos(k) * Math.sin(e) * Math.sin(d));
        g2 = Math.asin(Math.max(Math.min(g2, 1), -1));
        d = Math.atan2(E2, t);
        k = [Math.cos(u) * (Math.sin(e) * Math.sin(k) * Math.cos(x) - Math.cos(k) * Math.sin(x)), Math.cos(u) * Math.cos(e) * Math.cos(x), Math.cos(u) * (Math.cos(k) * Math.sin(e) * Math.cos(x) + Math.sin(x) * Math.sin(k))];
        e = [-Math.cos(g2) * Math.sin(d), Math.cos(g2) * Math.cos(d)];
      ...
      if (a || "multires" != z && "cubemap" != z) {
        if ("multires" != z) l2 = 2 * Math.atan(Math.tan(0.5 * l2) / (a.drawingBufferWidth / a.drawingBufferHeight)), l2 = 1 / Math.tan(0.5 * l2), a.uniform1f(c.psi, d), a.uniform1f(c.theta, g2), a.uniform1f(c.rot, h), a.uniform1f(c.f, l2), true === G && "equirectangular" == z && (a.bindTexture(a.TEXTURE_2D, c.texture), a.texImage2D(a.TEXTURE_2D, 0, a.RGB, a.RGB, a.UNSIGNED_BYTE, m)), a.drawArrays(a.TRIANGLES, 0, 6);
        else {
          k = a.drawingBufferWidth / a.drawingBufferHeight;
          e = 2 * Math.atan(Math.tan(l2 / 2) * a.drawingBufferHeight / a.drawingBufferWidth);
          e = 1 / Math.tan(e / 2);
          k = [e / k, 0, 0, 0, 0, e, 0, 0, 0, 0, 100.1 / -99.9, 20 / -99.9, 0, 0, -1, 0];
          for (e = 1; e < m.maxLevel && a.drawingBufferWidth > m.tileResolution * Math.pow(2, e - 1) * Math.tan(l2 / 2) * 0.707; ) e++;
          c.level = e;
          e = [1, 0, 0, 0, 1, 0, 0, 0, 1];
          e = ua(e, -h, "z");
          e = ua(e, -g2, "x");
          ...
          a.uniformMatrix4fv(c.perspUniform, false, new Float32Array(ma(k)));
          a.uniformMatrix4fv(
            c.cubeUniform,
            false,
            new Float32Array(ma(e))
          );
          h = [k[0] * e[0], k[0] * e[1], k[0] * e[2], 0, k[5] * e[4], k[5] * e[5], k[5] * e[6], 0, k[10] * e[8], k[10] * e[9], k[10] * e[10], k[11], -e[8], -e[9], -e[10], 0];
          c.nodeCache.sort(ja);
          if (200 < c.nodeCache.length && c.nodeCache.length > c.currentNodes.length + 50) for (k = c.nodeCache.splice(200, c.nodeCache.length - 200), e = 0; e < k.length; e++) a.deleteTexture(k[e].texture);
          c.currentNodes = [];
          e = "fbudlr".split("");
          for (k = 0; 6 > k; k++) u = new ka(wa[k], e[k], 1, 0, 0, m.fullpath), Ja(h, u, g2, d, l2);
          ...
          if (0 === S.length) {
            for (g2 = 0; g2 < c.currentNodes.length; g2++) if (d = c.currentNodes[g2], !d.texture && !d.textureLoad) {
              d.textureLoad = true;
              setTimeout(Ka, 0, d);
              break;
            }
      ...
      }, l2 = 1 / Math.tan(l2 / 2), l2 = l2 * A.clientWidth / 2 + "px", d = "perspective(" + l2 + ") translateZ(" + l2 + ") rotateX(" + g2 + "rad) rotateY(" + d + "rad) ", l2 = Object.keys(s2), g2 = 0; 6 > g2; g2++) if (h = R.querySelector(".pnlm-" + l2[g2] + "face")) h.style.webkitTransform = d + s2[l2[g2]], h.style.transform = d + s2[l2[g2]];
    };
    ...
    var S = [], La = function() {
      function c2() {
        ...
        var d2 = function() {
          if (0 < e.image.width && 0 < e.image.height) {
            ...
            e.callback(e.texture, true);
          } else e.callback(e.texture, false);
          S.length ? (c3 = S.shift(), e.loadTexture(c3.src, c3.texture, c3.callback)) : l2[g2++] = e;
        };
        this.image.addEventListener("load", d2);
        this.image.addEventListener("error", d2);
      }
      function d(a2, c3, d2, g3) {
        this.node = a2;
        this.src = c3;
        this.texture = d2;
        this.callback = g3;
      ...
      var g2 = 4, l2 = {}, k;
      c2.prototype.loadTexture = function(a2, c3, d2) {
        this.texture = c3;
        this.callback = d2;
        this.image.src = a2;
      };
      for (var h = 0; h < g2; h++) l2[h] = new c2();
      return function(c3, h2, m2, p2) {
        k = p2;
        p2 = a.createTexture();
        g2 ? l2[--g2].loadTexture(h2, p2, m2) : S.push(new d(c3, h2, p2, m2));
        return p2;
      };
    }();
  }
  var s = "attribute vec2 a_texCoord;varying vec2 v_texCoord;void main() {gl_Position = vec4(a_texCoord, 0.0, 1.0);v_texCoord = a_texCoord;}", l = "attribute vec3 a_vertCoord;attribute vec2 a_texCoord;uniform mat4 u_cubeMatrix;uniform mat4 u_perspMatrix;varying mediump vec2 v_texCoord;void main(void) {gl_Position = u_perspMatrix * u_cubeMatrix * vec4(a_vertCoord, 1.0);v_texCoord = a_texCoord;}", oa = "precision highp float;\nuniform float u_aspectRatio;\nuniform float u_psi;\nuniform float u_theta;\nuniform float u_f;\nuniform float u_h;\nuniform float u_v;\nuniform float u_vo;\nuniform float u_rot;\nconst float PI = 3.14159265358979323846264;\nuniform sampler2D u_image0;\nuniform sampler2D u_image1;\nuniform bool u_splitImage;\nuniform samplerCube u_imageCube;\nvarying vec2 v_texCoord;\nuniform vec4 u_backgroundColor;\nvoid main() {\nfloat x = v_texCoord.x * u_aspectRatio;\nfloat y = v_texCoord.y;\nfloat sinrot = sin(u_rot);\nfloat cosrot = cos(u_rot);\nfloat rot_x = x * cosrot - y * sinrot;\nfloat rot_y = x * sinrot + y * cosrot;\nfloat sintheta = sin(u_theta);\nfloat costheta = cos(u_theta);\nfloat a = u_f * costheta - rot_y * sintheta;\nfloat root = sqrt(rot_x * rot_x + a * a);\nfloat lambda = atan(rot_x / root, a / root) + u_psi;\nfloat phi = atan((rot_y * costheta + u_f * sintheta) / root);", qa = oa + "float cosphi = cos(phi);\ngl_FragColor = textureCube(u_imageCube, vec3(cosphi*sin(lambda), sin(phi), cosphi*cos(lambda)));\n}", pa = oa + "lambda = mod(lambda + PI, PI * 2.0) - PI;\nvec2 coord = vec2(lambda / PI, phi / (PI / 2.0));\nif(coord.x < -u_h || coord.x > u_h || coord.y < -u_v + u_vo || coord.y > u_v + u_vo)\ngl_FragColor = u_backgroundColor;\nelse {\nif(u_splitImage) {\nif(coord.x < 0.0)\ngl_FragColor = texture2D(u_image0, vec2((coord.x + u_h) / u_h, (-coord.y + u_v + u_vo) / (u_v * 2.0)));\nelse\ngl_FragColor = texture2D(u_image1, vec2((coord.x + u_h) / u_h - 1.0, (-coord.y + u_v + u_vo) / (u_v * 2.0)));\n} else {\ngl_FragColor = texture2D(u_image0, vec2((coord.x + u_h) / (u_h * 2.0), (-coord.y + u_v + u_vo) / (u_v * 2.0)));\n}\n}\n}", bb = "varying mediump vec2 v_texCoord;uniform sampler2D u_sampler;void main(void) {gl_FragColor = texture2D(u_sampler, v_texCoord);}";
  return { renderer: function(g2, l2, p2, s2) {
    return new Ba(g2, l2, p2, s2);
  } };
}(window, document);
  ...
  function Ba(s, l) {
    function oa() {
      ...
      if (1 == a2.getElementsByTagName("i").length) K();
        ...
        if ("cubemap" == b.type) for (var n = 6, c2 = function() {
          ...
          ea.href = F(a3.target.src, true);
          ea.textContent = ea.href;
          K(b.strings.fileAccessError.replace("%s", ea.outerHTML));
        }, a2 = 0; a2 < P.length; a2++) f = b.cubeMap[a2], "null" == f ? (console.log("Will use background instead of missing cubemap face " + a2), c2()) : (b.basePath && !qa(f) && (f = b.basePath + f), P[a2].onload = c2, P[a2].onerror = e2, P[a2].src = F(f));
          ...
          d2.responseType = "blob";
          d2.setRequestHeader(
            "Accept",
            "image/*,*/*;q=0.9"
          );
          d2.withCredentials = "use-credentials" === b.crossOrigin;
          d2.send();
        }
    ...
    function pa() {
      ...
      Sa || (Sa = true, W.addEventListener("mousedown", ka, false), g.addEventListener("mousemove", ua, false), g.addEventListener("mouseup", ma, false), b.mouseZoom && (J.addEventListener(
        ...
        U,
        false
      ), J.addEventListener("DOMMouseScroll", U, false)), b.doubleClickZoom && W.addEventListener("dblclick", Ja, false), s.addEventListener("mozfullscreenchange", e, false), s.addEventListener("webkitfullscreenchange", e, false), s.addEventListener("msfullscreenchange", e, false), s.addEventListener("fullscreenchange", e, false), E.addEventListener("resize", z, false), E.addEventListener("orientationchange", z, false), b.disableKeyboardCtrl || (s.addEventListener("keydown", V, false), s.addEventListener("keyup", R, false), s.addEventListener("blur", $, false)), g.addEventListener(
        "mouseleave",
        ma,
        false
    ...
    function Ba2(a2) {
      ...
      f.addEventListener("loadend", function() {
        ...
        if (-1 < c2 && true !== b.ignoreGPanoXMP) {
          var e2 = n.substring(c2, n.indexOf("</x:xmpmeta>") + 12), d2 = function(a3) {
            var b2;
            0 <= e2.indexOf(a3 + '="') ? (b2 = e2.substring(e2.indexOf(a3 + '="') + a3.length + 2), b2 = b2.substring(0, b2.indexOf('"'))) : 0 <= e2.indexOf(a3 + ">") && (b2 = e2.substring(e2.indexOf(a3 + ">") + a3.length + 1), b2 = b2.substring(
              0,
              b2.indexOf("<")
            ));
            return b2 !== p ? Number(b2) : null;
          }, n = d2("GPano:FullPanoWidthPixels"), c2 = d2("GPano:CroppedAreaImageWidthPixels"), g2 = d2("GPano:FullPanoHeightPixels"), h2 = d2("GPano:CroppedAreaImageHeightPixels"), l2 = d2("GPano:CroppedAreaTopPixels"), k2 = d2("GPano:PoseHeadingDegrees"), m2 = d2("GPano:PosePitchDegrees"), d2 = d2("GPano:PoseRollDegrees");
          null !== n && null !== c2 && null !== g2 && null !== h2 && null !== l2 && (0 > aa.indexOf("haov") && (b.haov = c2 / n * 360), 0 > aa.indexOf("vaov") && (b.vaov = h2 / g2 * 180), 0 > aa.indexOf("vOffset") && (b.vOffset = -180 * ((l2 + h2 / 2) / g2 - 0.5)), null !== k2 && 0 > aa.indexOf("northOffset") && (b.northOffset = k2, false !== b.compass && (b.compass = true)), null !== m2 && null !== d2 && (0 > aa.indexOf("horizonPitch") && (b.horizonPitch = m2), 0 > aa.indexOf("horizonRoll") && (b.horizonRoll = d2)));
        }
    ...
    function ta(a2) {
      ...
      a2 = f.x / n * 2 - 1;
      var c2 = (1 - f.y / c2 * 2) * c2 / n, d2 = 1 / Math.tan(b.hfov * Math.PI / 360), e2 = Math.sin(b.pitch * Math.PI / 180), g2 = Math.cos(b.pitch * Math.PI / 180), f = d2 * g2 - c2 * e2, n = Math.sqrt(a2 * a2 + f * f), c2 = 180 * Math.atan((c2 * g2 + d2 * e2) / n) / Math.PI;
    ...
    function la(a2) {
      if (b.draggable && (a2.preventDefault(), H && (N = Date.now()), ha && H)) {
    ...
    function A(a2) {
      "touch" == a2.pointerType && H && b.draggable && (ia.push(a2.pointerId), za.push({ clientX: a2.clientX, clientY: a2.clientY }), a2.targetTouches = za, Ka(a2), a2.preventDefault());
    }
    function c(a2) {
    ...
    function U(a2) {
      H && ("fullscreenonly" != b.mouseZoom || Aa) && (a2.preventDefault(), t(), N = Date.now(), a2.wheelDeltaY ? (x(b.hfov - 0.05 * a2.wheelDeltaY), w.hfov = 0 > a2.wheelDelta ? 1 : -1) : a2.wheelDelta ? (x(b.hfov - 0.05 * a2.wheelDelta), w.hfov = 0 > a2.wheelDelta ? 1 : -1) : a2.detail && (x(b.hfov + 1.5 * a2.detail), w.hfov = 0 < a2.detail ? 1 : -1), G());
    }
    function V(a2) {
      t();
      N = Date.now();
    ...
    function wa(a2, b2) {
      ...
      n && b2 && (ba = "undefined" !== typeof performance && performance.now() ? performance.now() : Date.now(), G());
    }
    function ga() {
      if (H) {
        var a2 = false, f = b.pitch, n = b.yaw, c2 = b.hfov, d2;
        d2 = "undefined" !== typeof performance && performance.now() ? performance.now() : Date.now();
        ba === p && (ba = d2);
        var e2 = (d2 - ba) * b.hfov / 1700, e2 = Math.min(e2, 1);
        r[0] && true === b.keyboardZoom && (x(b.hfov + (0.8 * w.hfov + 0.5) * e2), a2 = true);
        r[1] && true === b.keyboardZoom && (x(b.hfov + (0.8 * w.hfov - 0.2) * e2), a2 = true);
        if (r[2] || r[6]) b.pitch += (0.8 * w.pitch + 0.2) * e2, a2 = true;
        if (r[3] || r[7]) b.pitch += (0.8 * w.pitch - 0.2) * e2, a2 = true;
        if (r[4] || r[8]) b.yaw += (0.8 * w.yaw - 0.2) * e2, a2 = true;
        if (r[5] || r[9]) b.yaw += (0.8 * w.yaw + 0.2) * e2, a2 = true;
        a2 && (N = Date.now());
        if (b.autoRotate) {
          if (1e-3 < d2 - ba) {
            var a2 = (d2 - ba) / 1e3, g2 = (w.yaw / a2 * e2 - 0.2 * b.autoRotate) * a2, g2 = (0 < -b.autoRotate ? 1 : -1) * Math.min(Math.abs(b.autoRotate * a2), Math.abs(g2));
            b.yaw += g2;
          }
          b.autoRotateStopDelay && (b.autoRotateStopDelay -= d2 - ba, 0 >= b.autoRotateStopDelay && (b.autoRotateStopDelay = false, Z = b.autoRotate, b.autoRotate = 0));
        }
        O.pitch && (m("pitch"), f = b.pitch);
        O.yaw && (m("yaw"), n = b.yaw);
        O.hfov && (m("hfov"), c2 = b.hfov);
        0 < e2 && !b.autoRotate && (a2 = 1 - b.friction, r[4] || r[5] || r[8] || r[9] || O.yaw || (b.yaw += w.yaw * e2 * a2), r[2] || r[3] || r[6] || r[7] || O.pitch || (b.pitch += w.pitch * e2 * a2), r[0] || r[1] || O.hfov || x(b.hfov + w.hfov * e2 * a2));
        ba = d2;
        0 < e2 && (w.yaw = 0.8 * w.yaw + (b.yaw - n) / e2 * 0.2, w.pitch = 0.8 * w.pitch + (b.pitch - f) / e2 * 0.2, w.hfov = 0.8 * w.hfov + (b.hfov - c2) / e2 * 0.2, f = b.autoRotate ? Math.abs(b.autoRotate) : 5, w.yaw = Math.min(
          f,
          Math.max(w.yaw, -f)
    ...
    function ca() {
      if (!Za) if (Fa(), Qa && clearTimeout(Qa), ha || true === X) requestAnimationFrame(ca);
        ...
        0 < a2 ? Qa = setTimeout(function() {
          ...
          G();
        }, a2) : 0 <= b.autoRotateInactivityDelay && Z && (b.autoRotate = Z, da.lookAt(Ga, p, ra, 3e3), G());
      }
    }
    function Fa() {
      var a2;
      if (H) {
        ...
        if (b.avoidShowingBackground) {
          var c2 = b.hfov / 2, e2 = 180 * Math.atan2(Math.tan(c2 / 180 * Math.PI), f.width / f.height) / Math.PI;
          b.vaov > b.haov ? Math.min(Math.cos((b.pitch - c2) / 180 * Math.PI), Math.cos((b.pitch + c2) / 180 * Math.PI)) : n = c2 * (1 - Math.min(Math.cos((b.pitch - e2) / 180 * Math.PI), Math.cos((b.pitch + e2) / 180 * Math.PI)));
        }
        var c2 = b.maxYaw - b.minYaw, e2 = -180, d2 = 180;
        360 > c2 && (e2 = b.minYaw + b.hfov / 2 + n, d2 = b.maxYaw - b.hfov / 2 - n, c2 < b.hfov && (e2 = d2 = (e2 + d2) / 2), b.yaw = Math.max(e2, Math.min(d2, b.yaw)));
        false === b.autoRotate && (360 < b.yaw ? b.yaw -= 360 : -360 > b.yaw && (b.yaw += 360));
    ...
    function La(a2) {
      ...
      var e2;
      if (a2.video) {
        e2 = g.createElement("video");
        var d2 = a2.video;
        b.basePath && !qa(d2) && (d2 = b.basePath + d2);
        e2.src = F(d2);
        e2.controls = true;
        e2.style.width = a2.width + "px";
        M.appendChild(f);
        c2.appendChild(e2);
      } else if (a2.image) {
        d2 = a2.image;
        b.basePath && !qa(d2) && (d2 = b.basePath + d2);
        e2 = g.createElement("a");
        e2.href = F(a2.URL ? a2.URL : d2, true);
        e2.target = "_blank";
        c2.appendChild(e2);
        ...
        e2 = g.createElement("a");
        e2.href = F(a2.URL, true);
        if (a2.attributes) for (d2 in a2.attributes) d2 = String(d2).toLowerCase().replace(
          /[^a-z]/g,
          ""
        ), l.escapeHTML && (d2.startsWith("on") || d2.includes("href")) ? console.log("Hot spot attribute skipped.") : e2.setAttribute(d2, a2.attributes[d2]);
        else e2.target = "_blank";
        M.appendChild(e2);
      ...
      if (a2.createTooltipFunc) a2.createTooltipFunc(
        f,
        a2.createTooltipArgs
      );
    ...
    function Ca(a2) {
      var f = Math.sin(a2.pitch * Math.PI / 180), c2 = Math.cos(a2.pitch * Math.PI / 180), e2 = Math.sin(b.pitch * Math.PI / 180), d2 = Math.cos(b.pitch * Math.PI / 180), g2 = Math.cos((-a2.yaw + b.yaw) * Math.PI / 180), h2 = f * e2 + c2 * g2 * d2;
      if (90 >= a2.yaw && -90 < a2.yaw && 0 >= h2 || (90 < a2.yaw || -90 >= a2.yaw) && 0 >= h2) a2.div.style.visibility = "hidden";
        ...
        var m2 = C.getCanvas(), p2 = m2.clientWidth, m2 = m2.clientHeight, f = [-p2 / k2 * l2 * c2 / h2 / 2, -p2 / k2 * (f * d2 - c2 * g2 * e2) / h2 / 2], c2 = Math.sin(b.roll * Math.PI / 180), e2 = Math.cos(b.roll * Math.PI / 180), f = [f[0] * e2 - f[1] * c2, f[0] * c2 + f[1] * e2];
    ...
    function I(a2) {
      b = {};
      var f, c2, e2 = "haov vaov vOffset northOffset horizonPitch horizonRoll".split(" ");
      aa = [];
      for (f in Va) Va.hasOwnProperty(f) && (b[f] = Va[f]);
      for (f in l.default) if (l.default.hasOwnProperty(f)) if ("strings" == f) for (c2 in l.default.strings) l.default.strings.hasOwnProperty(c2) && (b.strings[c2] = D(l.default.strings[c2]));
      else b[f] = l.default[f], 0 <= e2.indexOf(f) && aa.push(f);
      if (null !== a2 && "" !== a2 && l.scenes && l.scenes[a2]) {
        var d2 = l.scenes[a2];
        for (f in d2) if (d2.hasOwnProperty(f)) if ("strings" == f) for (c2 in d2.strings) d2.strings.hasOwnProperty(c2) && (b.strings[c2] = D(d2.strings[c2]));
        else b[f] = d2[f], 0 <= e2.indexOf(f) && aa.push(f);
        b.scene = a2;
      }
      for (f in l) if (l.hasOwnProperty(f)) if ("strings" == f) for (c2 in l.strings) l.strings.hasOwnProperty(c2) && (b.strings[c2] = D(l.strings[c2]));
      else b[f] = l[f], 0 <= e2.indexOf(f) && aa.push(f);
    }
    function k(a2) {
      if ((a2 = a2 ? a2 : false) && "preview" in b) {
        ...
        sa.style.backgroundImage = "url('" + F(c2).replace(/"/g, "%22").replace(/'/g, "%27") + "')";
        M.appendChild(sa);
      }
      var c2 = b.title, e2 = b.author;
      ...
      for (var d2 in b) if (b.hasOwnProperty(d2)) switch (d2) {
        case "title":
          q.title.innerHTML = D(b[d2]);
          q.container.style.display = "inline";
          break;
        case "author":
          var h2 = D(b[d2]);
          b.authorURL && (h2 = g.createElement("a"), h2.href = F(b.authorURL, true), h2.target = "_blank", h2.innerHTML = D(b[d2]), h2 = h2.outerHTML);
        ...
        case "hfov":
          ...
          break;
        case "autoLoad":
          true === b[d2] && C === p && (q.load.box.style.display = "inline", v.load.style.display = "none", oa());
          break;
        case "showZoomCtrl":
          v.zoom.style.display = b[d2] && false != b.showControls ? "block" : "none";
          break;
        case "showFullscreenCtrl":
          v.fullscreen.style.display = b[d2] && false != b.showControls && ("fullscreen" in g || "mozFullScreen" in g || "webkitIsFullScreen" in g || "msFullscreenElement" in g) ? "block" : "none";
          break;
        case "hotSpotDebug":
          Wa.style.display = b[d2] ? "block" : "none";
          break;
        case "showControls":
          b[d2] || (v.orientation.style.display = "none", v.zoom.style.display = "none", v.fullscreen.style.display = "none");
          break;
        case "orientationOnByDefault":
          b[d2] && Ra();
      }
      a2 && (c2 ? b.title = c2 : delete b.title, e2 ? b.author = e2 : delete b.author);
    }
    function h() {
      if (H && !Na) if (Aa) g.exitFullscreen ? g.exitFullscreen() : g.mozCancelFullScreen ? g.mozCancelFullScreen() : g.webkitCancelFullScreen ? g.webkitCancelFullScreen() : g.msExitFullscreen && g.msExitFullscreen();
      else try {
    ...
    function y(a2, c2, e2, h2, g2) {
      ...
      m2 = "same" === e2 ? b.yaw : "sameAzimuth" === e2 ? b.yaw + (b.northOffset || 0) - (l.scenes[a2].northOffset || 0) : e2;
    ...
    function D(a2) {
      return l.escapeHTML ? String(a2).split(/&/g).join("&amp;").split('"').join("&quot;").split("'").join("&#39;").split("<").join("&lt;").split(">").join("&gt;").split("/").join("&#x2f;").split("\n").join("<br>") : String(a2).split("\n").join("<br>");
    }
    function F(a2, b2) {
      try {
    ...
    var da = this, b, C, sa, ha = false, N = Date.now(), xa = 0, ya = 0, Ha = -1, Oa = 0, Pa = 0, r = Array(10), Aa = false, H, Na = false, Sa = false, P, ba, w = { yaw: 0, pitch: 0, hfov: 0 }, Ta = false, X = false, $a = 0, Qa, Z = 0, ra, Ga, O = {}, T = {}, aa = [], Ma = false, Ua = false, Za = false, Va = {
      hfov: 100,
      minHfov: 50,
    ...
    J.appendChild(Ia);
    l.firstScene ? I(l.firstScene) : l.default && l.default.firstScene ? I(l.default.firstScene) : I(null);
    k(true);
    var ia = [], za = [];
    ...
    this.setYaw = function(a2, c2, d2, e2) {
      ...
      a2 = (a2 + 180) % 360 - 180;
      c2 ? (180 < b.yaw - a2 ? a2 += 360 : 180 < a2 - b.yaw && (a2 -= 360), O.yaw = { startTime: Date.now(), startPosition: b.yaw, endPosition: a2, duration: c2 }, "function" == typeof d2 && setTimeout(
        function() {
          d2(e2);
        },
        c2
    ...
    this.setHfov = function(a2, c2, d2, e2) {
      ...
      (c2 = c2 == p ? 1e3 : Number(c2)) ? (O.hfov = { startTime: Date.now(), startPosition: b.hfov, endPosition: u(a2), duration: c2 }, "function" == typeof d2 && setTimeout(
        function() {
          d2(e2);
        },
        c2
    ...
    this.setHorizonRoll = function(a2) {
      b.horizonRoll = Math.min(90, Math.max(-90, a2));
    ...
    this.removeScene = function(a2) {
      if (b.scene === a2 || !l.scenes.hasOwnProperty(a2)) return false;
      delete l.scenes[a2];
      return true;
    };
    ...
    this.addHotSpot = function(a2, c2) {
      if (c2 === p && b.scene === p) b.hotSpots.push(a2);
        ...
        if (l.scenes.hasOwnProperty(d2)) l.scenes[d2].hasOwnProperty("hotSpots") || (l.scenes[d2].hotSpots = [], d2 == b.scene && (b.hotSpots = l.scenes[d2].hotSpots)), l.scenes[d2].hotSpots.push(a2);
        else throw "Invalid scene ID!";
      }
      if (c2 === p || b.scene == c2) La(a2), H && Ca(a2);
      return this;
    };
    this.removeHotSpot = function(a2, c2) {
      ...
      } else if (l.scenes.hasOwnProperty(c2)) {
        if (!l.scenes[c2].hasOwnProperty("hotSpots")) return false;
        for (d2 = 0; d2 < l.scenes[c2].hotSpots.length; d2++) if (l.scenes[c2].hotSpots[d2].hasOwnProperty("id") && l.scenes[c2].hotSpots[d2].id === a2) return l.scenes[c2].hotSpots.splice(d2, 1), true;
      } else return false;
    };
    this.resize = function() {
      C && z();
    ...
    this.destroy = function() {
      ...
      clearTimeout(Qa);
      C && C.destroy();
      Sa && (g.removeEventListener("mousemove", ua, false), g.removeEventListener("mouseup", ma, false), s.removeEventListener("mozfullscreenchange", e, false), s.removeEventListener("webkitfullscreenchange", e, false), s.removeEventListener("msfullscreenchange", e, false), s.removeEventListener("fullscreenchange", e, false), E.removeEventListener(
        "resize",
        z,
        false
      ), E.removeEventListener("orientationchange", z, false), s.removeEventListener("keydown", V, false), s.removeEventListener("keyup", R, false), s.removeEventListener("blur", $, false), g.removeEventListener("mouseleave", ma, false));
      s.innerHTML = "";
      s.classList.remove("pnlm-container");
    };
  }
  return { viewer: function(g2, l) {
    return new Ba(g2, l);
  } };
}(window, document);
